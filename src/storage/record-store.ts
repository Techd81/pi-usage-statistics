...
export class RecordStore {
  ...
  /** Upsert one record by recordId and mark the store dirty (flush on stop). */
  ...
  /**
   * Persist the in-memory state atomically. When the record count exceeds
   * maxRecords, compact (rewrite the deduped set) so the file stays bounded.
   */
  async flush(): Promise<void> {
    await mkdir(this.storeDir, { recursive: true });
    if (this.records.length > this.maxRecords) {
      this.records = [...indexByRecordId(this.records).values()];
    }
